...
function filterAndSortData() {
    ...
    filtered.sort((a,b) => {
        ...
        if(sortColumn==="pilot"||sortColumn==="country")
...
function renderTable(){
    ...
    allSorted.forEach((p) => {
        ...
        html += `<tr class="${rowClass}">
            ...
            <td><span class="time-cell">⏱️ ${p.bestLap.toFixed(3)} s</span></td>
            <td class="points-cell"><i class="fas fa-star"></i> ${p.points}</td>
...
function renderPublicHeats(){
    ...
    for(let heatNum in groups){
        ...
        groups[heatNum].forEach(p => {
            html += `<li>🏁 ${escapeHtml(p.name)} (${p.countryCode}) – ${p.points} очков</li>`;
        });
        html += `</ul></div>`;
    }
...
function renderAdminList(){
    ...
    pilotsData.forEach(pilot => {
        ...
        card.innerHTML = `
            ...
            <small>🏆 ${pilot.points} pts | ⏱️ ${pilot.bestLap.toFixed(3)}s ${pilot.disqualified ? '| ⛔ ДИСКВ.' : ''}</small></div>
...
function startEditPilot(id){
    ...
    editBestLap.value = p.bestLap;
    editPoints.value = p.points;
...
function addOrUpdatePilot(){
    ...
    const lap = parseFloat(editBestLap.value);
    const pts = parseInt(editPoints.value);
    if(!name || !cc || !cn || isNaN(lap) || isNaN(pts)){ formMessage.innerHTML = "❌ Заполните все поля!"; return; }
...
function renderHeatsAssignment(){
    ...
    let html = `<div style="font-weight:bold; margin-bottom:8px;">Выберите номер залёта для каждого пилота (0 – не участвует)</div>`;
    pilotsData.forEach(pilot => {
        if(pilot.disqualified) return;
        ...
        html += `<div class="pilot-heat-row">
            ...
            <select data-id="${pilot.id}" class="heat-select">
                ${[0,1,2,3,4,5,6].map(num => `<option value="${num}" ${currentHeat==num ? 'selected' : ''}>${num === 0 ? '— не назначен' : `Залёт №${num}`}</option>`).join('')}
            </select>
        </div>`;
    });
